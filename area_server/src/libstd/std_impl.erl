...
create_area() ->
    ...
    %monsrv_rpc(mon, register_node, [node()]),
    ok.

%%----------------------------------------------------------------------
%% @spec join_area() -> ok
%% @doc
%% This function joins an existing area in cluster. The area must have
%% been created with the function create_area/0 in another computer.
%% There is no need to specify what computer to connected to, this is
%% automatically discovered.
%% @end
%%----------------------------------------------------------------------
join_area() ->
    Node = libdist:join_area(),
    libgeo:join(Node),
