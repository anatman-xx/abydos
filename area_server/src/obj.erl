...
%% @end
%%----------------------------------------------------------------------
init(#obj{id=Id, type=Type} = State) ->
    libstd.srv:register_obj(Id, self()),
    {ok, State}.

post_init(_From, #obj{id=Id} = State) ->
...
notify_flying(From, _Id, _Mode, State) ->
    {noreply, State}.


info(From, #obj{id=Id, type=Type, parents=Parents} = State) ->
...
set_pos(_From, Pos, State) ->
    {ok, _Reply, NewState} = obj:call_self(set_property, [pos, Pos], State),
    {noreply, NewState}.
