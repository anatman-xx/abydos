...
handle_call({save_player, Id, Account, Name, ObjState}, _From, 
    #state{mod=Mod} = State) ->
    Result = Mod:save_player(Id, Account, Name, ObjState),
    {reply, Result, State};
...
save_player(Id, Account, Name, State) ->
    gen_server:call(?MODULE, {save_player, Id, Account, Name, State}).
