...
init() ->
    ...
    case lists:member(shared_obj_registry, mnesia:system_info(tables)) of
        true ->
...
join_area() ->
    Node = std_funs:monsrv_rpc(monsrv, get_area, [std_funs:area_name()]),
...
lookup_neighbours(AreaId) ->
    lookup_neighbours(AreaId, std_funs:neighbours()).
